...
import net.minecraftforge.fml.common.event.{FMLPreInitializationEvent, FMLStateEvent}
...
trait Register {
	...
	final def findFreeBiomeID(exclude: Int*): Int = {
		val biomes = BiomeGenBase.getBiomeGenArray
		biomes.indices.find(i => {
			biomes(i) == null && !exclude.contains(i)
		}).getOrElse(throw new Exception("Error: " + this.getMod.getModName +
				" requested a free biome ID, but there is no room in the BiomeGenBase array! Eeeeek!"))
	}
				            ...
				            RegisterPhase.PRE_INIT -> ((reg: Register) => {
					            ...
					            reg match {
						            case blockReg: BlockRegister =>
							            ...
							            Registry.addBlockRegister(blockReg)
						            case _ =>
					            }
				            }),
				            // init phase
