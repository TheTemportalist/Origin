...
import temportalist.origin.api.common.utility.{Players, Teleport}
 ...
 */
object CommandOrigin extends CommandBase with ICommand {
	...
	override def getCommandAliases: util.List[String] = this.aliases

	override def getUsage: String = "commands.origin.usage"
	...
	override def processCommand(sender: ICommandSender, args: Array[String]): Unit = {
		...
		if (commandType.equals("tp")) {
			...
			sender match {
				case player1: EntityPlayer =>
					...
					if (hasAlternatePlayer) {
						coordStartIndex += 1
						player = Players.getPlayerOnline(args(1))
					}
			...
			val player: EntityPlayer = Players.getPlayerOnline(args(1))
			val amount: Int =
				try {
					args(3).toInt
				}
