...
import temportalist.origin.api.client.utility.TessRenderer
import temportalist.origin.api.common.lib.IRadialSelection
import temportalist.origin.foundation.client.gui.GuiRadialMenu.RadialHandler
import temportalist.origin.foundation.common.network.PacketTriggerRadialSelection
...
abstract class GuiRadialMenu[T <: IRadialSelection](
	...
	def renderMenu(): Unit = {
		...
		val resolution: ScaledResolution = new ScaledResolution(this.mc)
	...
	def renderRadial(resolution: ScaledResolution, zLevel: Double,
		...
		*/
		...
		for (i <- 0 until quantity) {
			...
			TessRenderer.startQuads(DefaultVertexFormats.POSITION)

			if (isMouseIn) {
				TessRenderer.getRenderer.putColorRGB_F(
					28F / 255F, 232F / 255F, 31F / 255F, (153F / 255F).toInt)
				this.selectedLocalIndex = i
			}
			else {
				TessRenderer.getRenderer.putColorRGB_F(
					0F / 255F, 0F / 255F, 0F / 255F, (153F / 255F).toInt)
			}

			TessRenderer.getRenderer.pos(
				Math.cos(currAngle) *
	...
	def renderIconsAndText(resolution: ScaledResolution, zLevel: Double,
		...
		for (i <- 0 until quantity) {
			...
			if (selection != null) {
				...
				selection.draw(this.mc, iconX, iconY, zLevel, size, size, renderText = true)
			}
		}
